...
| **Ứng dụng Điện thoại bị hệ thống khai tử (Killed)** | Cơ chế tối ưu pin của Android rất nghiêm ngặt đối với ứng dụng chạy ngầm dài hạn. | 1. Yêu cầu người dùng tắt "Tối ưu hóa pin" (Battery Optimization) cho ứng dụng.<br>2. Chạy dịch vụ dưới dạng **Foreground Service** hiển thị một thông báo cố định trên thanh trạng thái (Sticky Notification).<br>3. **Whitelist battery (từ v1.5.7)**: `BatteryOptimizationSection` (shared widget) trong Settings của cả phone lẫn TV — bọc `BatteryOptimizationService` (MethodChannel `/methods` → `NativeBridgePlugin`) trả về `isIgnoringBatteryOptimizations()` và mở `ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` (hoặc fallback `ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS`). |
| **ROM Trung Quốc (MIUI/HyperOS, ColorOS, EMUI/HarmonyOS, OriginOS...) kill service dù đã whitelist AOSP** | Các OEM có lớp quản lý pin riêng dưới tầng Android API: chặn autostart, đóng băng app nền, quét RAM — làm chết FGS và `NotificationListenerService` kể cả khi đã tắt battery optimization chuẩn. | Không có API công khai ép được. Người dùng phải cấu hình tay trên ROM: (1) bật **Autostart** (自启动), (2) chọn **No restrictions / Allow background activity** trong app battery saver, (3) **khoá app trong Recents** tránh bị "Clear all" quét, (4) cấp notification access cho `NotificationListenerService`. MIUI có thêm quyền riêng "hiển thị cửa sổ nổi khi chạy nền" cần bật cho overlay TV. Chi tiết theo hãng: https://dontkillmyapp.com |
| **TV Service bị Android kill** | Android có thể kill Foreground Service khi thiếu RAM hoặc do chính sách OEM. | Cơ chế **Watchdog AlarmManager**: cứ mỗi 5 phút, một `PendingIntent` qua `RestartAlarmReceiver` sẽ kiểm tra và khởi động lại `BackgroundService` nếu nó không chạy (idempotent). Không phụ thuộc vào tiến trình Flutter, hoạt động ở native layer. |
| **TV overlay không hiện sau khi swipe app khỏi đa nhiệm** | Overlay được vẽ từ `MainActivity` (Flutter UI isolate). Khi task bị xoá, Activity/UI isolate bị destroy nên không còn ai xử lý yêu cầu hiển thị overlay, dù FGS (và WebSocket server) vẫn sống. | Chuyển việc vẽ overlay sang **`OverlayManager` (singleton native)** và đăng ký receiver ở **`TvApplication`** (sống theo tiến trình, không theo Activity). Background isolate gửi broadcast `ACTION_SHOW_OVERLAY` trực tiếp qua `native_bridge` plugin (đăng ký trên cả UI lẫn background engine) thay vì `service.invoke()` qua UI isolate. |
| **Phone bị hệ thống khởi động lại chỉ để gửi thông báo** | Khi process bị kill, hệ thống restart `NotificationListenerService` để chuyển thông báo, nhưng Flutter background isolate (giữ receiver + WebSocket) chưa chạy → broadcast bị rớt. | `MyNotificationListener.onNotificationPosted()` gọi `startForegroundService(BackgroundService)` trước khi broadcast, đảm bảo pipeline khôi phục ngay (không chờ watchdog). |
| **Phone mất kết nối WebSocket** | WebSocket bị đứt do TV restart, network glitch, hoặc TV bị kill. | **Exponential backoff reconnect** trên phone: 5s → 10s → 20s → 40s → 60s (cap). Tự động reset về 5s khi kết nối lại thành công. |
| **Khó gỡ lỗi pipeline trên device thật** | Các sự kiện quan trọng (connect/reconnect, gửi/nhận notification, overlay show/hide) chạy trong background isolate, không thấy được từ log hệ thống khi debug trên device thật. | **In-app Debug Log**: `ConnectorService`/`ServerService` phát `logStream`; background isolate chuyển qua `service.invoke('debugLog', ...)`; provider `DebugLog`/`TvDebugLog` (ring buffer 300 dòng) lắng nghe và render bởi `DebugLogScreen` (mở từ Settings). **Debug Status HUD (TV)**: toggle trong Settings → background isolate broadcast `ACTION_SHOW/UPDATE/HIDE_STATUS_OVERLAY` → `OverlayManager` vẽ HUD native luôn nổi (server state, số client connected/paired, DND) mỗi 1s, hoạt động cả khi app bị kill. |
